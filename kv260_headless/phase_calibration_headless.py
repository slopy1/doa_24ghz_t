    ...
    def read_samples(self, num_samples: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        while samples_read < num_samples:
            ...
            if sr.ret > 0:
                samples_read += sr.ret
                retries = 0
            elif sr.ret in (0, -1, -4):
                # timeout/overflow are advisory in SoapyBladeRF; retry boundedly.
                retries += 1
                if retries > max_retries:
                    raise RuntimeError(
                        f"calibration stream stalled after {samples_read}/{num_samples} "
                        f"samples (last ret={sr.ret})"
                    )
                continue
            elif sr.ret < 0:
                raise RuntimeError(
                    f"calibration stream error ret={sr.ret} "
                    f"after {samples_read}/{num_samples} samples"
                )
...
def bandpass_filter(signal: np.ndarray, sample_rate: float,
    ...
    # Create bandpass mask (one-sided: complex baseband, +f and -f are independent)
...
def write_calibration_file(phase_deg: float, config: CalibrationConfig) -> None:
    """Persist calibration for direct workflows that do not pass --cal."""
    cal_path = Path(__file__).parent / "calibration.json"
    payload = {
        "phase_offset_deg": round(float(phase_deg), 2),
        "timestamp": datetime.now().isoformat(),
        "gain_setting": config.RX_GAIN,
        "frequency_hz": config.CENTER_FREQ,
    }
    try:
        cal_path.write_text(json.dumps(payload, indent=2) + "\n")
        print(f"# Wrote calibration: {cal_path}", flush=True)
    except OSError as e:
        print(f"# WARNING: failed to write calibration file {cal_path}: {e}",
              flush=True)
...
def run_calibration(config: CalibrationConfig):
    ...
    try:
        ...
        if phase_estimates:
            ...
            write_calibration_file(final_phase, config)
            sys.stdout.flush()
        else:
