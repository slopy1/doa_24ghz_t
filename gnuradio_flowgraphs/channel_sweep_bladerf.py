...
# Title: BladeRF AoA Channel Sweep
# Author: DoA Thesis Project
# Description: Direction of Arrival estimation with WiFi channel selection
...
class channel_sweep_bladerf(gr.top_block, Qt.QWidget):

    def __init__(self):
        gr.top_block.__init__(self, "BladeRF AoA Channel Sweep", catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle("BladeRF AoA Channel Sweep")
        ...
        self.settings = Qt.QSettings("gnuradio/flowgraphs", "channel_sweep_bladerf")
        ...
        self.center_freq = center_freq = (2412e6 + (wifi_channel - 1) * 5e6) if wifi_channel <= 13 else 2484e6
        ...
        self.antenna_spacing = antenna_spacing = 0.0612 / (3e8 / center_freq)
        ...
        self._wifi_channel_options = [1, 2, 3, 4, 5]
        # Create the labels list
        self._wifi_channel_labels = ['Ch 1 (2412 MHz)', 'Ch 2 (2417 MHz)', 'Ch 3 (2422 MHz)', 'Ch 4 (2427 MHz)', 'Ch 5 (2432 MHz)']
        # Create the combo box
        self._wifi_channel_tool_bar = Qt.QToolBar(self)
        self._wifi_channel_tool_bar.addWidget(Qt.QLabel("WiFi Channel" + ": "))
        self._wifi_channel_combo_box = Qt.QComboBox()
        self._wifi_channel_tool_bar.addWidget(self._wifi_channel_combo_box)
        for _label in self._wifi_channel_labels: self._wifi_channel_combo_box.addItem(_label)
        self._wifi_channel_callback = lambda i: Qt.QMetaObject.invokeMethod(self._wifi_channel_combo_box, "setCurrentIndex", Qt.Q_ARG("int", self._wifi_channel_options.index(i)))
        self._wifi_channel_callback(self.wifi_channel)
        self._wifi_channel_combo_box.currentIndexChanged.connect(
            lambda i: self.set_wifi_channel(self._wifi_channel_options[i]))
        # Create the radio buttons
        self.top_grid_layout.addWidget(self._wifi_channel_tool_bar, 0, 0, 1, 1)
        for r in range(0, 1):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(0, 1):
            self.top_grid_layout.setColumnStretch(c, 1)
        ...
        self.top_grid_layout.addWidget(self._qtgui_vector_sink_f_0_win, 5, 0, 1, 2)
        for r in range(5, 6):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(0, 2):
            self.top_grid_layout.setColumnStretch(c, 1)
        ...
        self.top_grid_layout.addWidget(self._qtgui_time_sink_x_0_win, 3, 0, 2, 2)
        for r in range(3, 5):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(0, 2):
            self.top_grid_layout.setColumnStretch(c, 1)
        ...
        self.top_grid_layout.addWidget(self._qtgui_number_sink_0_win, 2, 0, 1, 2)
        for r in range(2, 3):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(0, 2):
            self.top_grid_layout.setColumnStretch(c, 1)
...
def main(top_block_cls=channel_sweep_bladerf, options=None):
