#!/usr/bin/env python3
"""
aoa_estimation_fpga.py - FPGA-Accelerated DoA Estimation for BladeRF System

Same as aoa_estimation_headless.py but offloads cross-correlation to the
xcorr_acc FPGA IP via AXI DMA. The ARM still runs the DoA algorithms
(Root-MUSIC, MUSIC, MVDR, Phase Diff) using the FPGA-computed R[0,1].
...
Usage:
    sudo python3 aoa_estimation_fpga.py --cal=-12.5 --algo=ROOTMUSIC
...
def run_estimation(config: FPGAConfig):
    ...
    print(f"# FPGA-accelerated DoA estimation")
    ...
    try:
        ...
        while True:
            ...
            # Also compute ARM reference for debug
            arm_acc_re = 0.0
            arm_acc_im = 0.0
            ...
            for k in range(n_snap):
                ...
                # Power on ARM (for covariance matrix diagonal)
                acc_r00 += float(np.sum(np.abs(ch0_snap)**2))
                acc_r11 += float(np.sum(np.abs(ch1_snap)**2))

                # ARM reference cross-correlation
                if config.DEBUG and iteration < 3:
                    xcorr_arm = np.sum(ch0_snap * np.conj(ch1_snap))
                    arm_acc_re += xcorr_arm.real
                    arm_acc_im += xcorr_arm.imag
            ...
            if config.DEBUG and iteration < 3:
                arm_avg_re = arm_acc_re / n_snap
                arm_avg_im = arm_acc_im / n_snap
                scale = 32767.0
                # ARM xcorr in SC16 units for comparison
                arm_sc16_re = arm_avg_re * scale * scale
                arm_sc16_im = arm_avg_im * scale * scale
                fpga_phase = math.degrees(math.atan2(xcorr_im, xcorr_re))
                arm_phase = math.degrees(math.atan2(arm_avg_im, arm_avg_re))
                print(f"# DEBUG iter={iteration}:")
                print(f"#   FPGA xcorr: re={xcorr_re:.0f} im={xcorr_im:.0f} phase={fpga_phase:.1f}")
                print(f"#   ARM  xcorr: re={arm_sc16_re:.0f} im={arm_sc16_im:.0f} phase={arm_phase:.1f}")
                print(f"#   CF32 range: ch0=[{np.min(np.abs(ch0_all)):.6f}, {np.max(np.abs(ch0_all)):.6f}]")
                print(f"#   SC16 range: [{int(np.min(np.real(ch0_all[:ss]))*scale)}, {int(np.max(np.real(ch0_all[:ss]))*scale)}]")
            ...
            if algorithm == "PHASEDIFF":
                ...
                # Build averaged covariance matrix
                scale = 32767.0
                r00 = acc_r00 * scale * scale / (n_snap * ss)
                r11 = acc_r11 * scale * scale / (n_snap * ss)
...
def main():
    parser = argparse.ArgumentParser(
        description="FPGA-accelerated DoA estimation for BladeRF system"
    )
    ...
    print("# FPGA DoA Estimation starting")
