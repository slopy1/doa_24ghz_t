...
class Config:
    ...
    ESTIMATION_FPGA_SCRIPT = SCRIPT_DIR / "aoa_estimation_fpga_kv260.py"
...
class DoAController:
    ...
    def cmd_estimate(self, algo: str = None):
        ...
        if not self.calibration.is_valid():
            self.send("WARN:No calibration — running with cal=0.0°")
        ...
        def on_output(line: str):
            ...
            if line.startswith("AOA:"):
                ...
                # Surface anything else (banners, tracebacks, INFO) so we can debug
                self.log(f"[subproc] {line}")
